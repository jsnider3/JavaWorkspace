...
public class DiceRollerImproved {
	...
	JPanel _Panel;
	JLabel _DiceLabel;
	JTextField _DiceText;
	JLabel _ResultsLabel;
	JTextField _ResultsText;
	JLabel _Again;
	JTextField _AgainText;
	JMenuBar _MenuBar;
	JMenu _HelpMenu;
	JMenuItem _HelpMenuButton;
	JButton _RollButton;
	JButton _CalculateButton;
	JTextArea _HelpText;
	//OddsCalculator test;// = new OddsCalculator();
	...
	public int getAgain(){
		int tAgain;
		try{
			tAgain = Integer.parseInt(_AgainText.getText()) - 1;
			if(tAgain < 7 || tAgain > 9){
				tAgain = 9;
			}
		}catch(Exception e){
			tAgain = 9;
	...
	class CalculateListener implements ActionListener{
		public void actionPerformed(ActionEvent arg0){
			...
			int tExpected = (int)(tDice * 0.3 + 0.3 * (10 - tAgain));
			...
			tBuilder.append("Dice=" + tDice + " Again=" + (tAgain + 1) + "\n");
			for(int x = 0; x < 10; x++){
	...
	class helpListener implements ActionListener{
		public void actionPerformed(ActionEvent arg0) {
			if(_HelpMenuButton.getText() == "Help"){
				_HelpText.setText(
